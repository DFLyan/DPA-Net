...
def train():
    ...
    t2, sp1, sp2, sp3, sp4 = T2(y1_image, t2_a1, t2_a2, t2_a3, t2_a4, is_train=True, reuse=False)
    t = add_two_layer(t1, t2)


    t1_, t2_a1_, t2_a2_, t2_a3_, t2_a4_ = T1(y1_image, is_train=False, reuse=True)
    t2_, sp1_, sp2_, sp3_, sp4_ = T2(y1_image, t2_a1_, t2_a2_, t2_a3_, t2_a4_, is_train=False, reuse=True)
    ...
    g_loss_t2 = ade_loss_t2 + meas_loss
    ...
    t2_optim = tf.train.AdamOptimizer(lr_v_2).minimize(g_loss_t2, var_list=t2_vars)
    ...
    writer_1 = tf.summary.FileWriter("logs/DPA/%s/c%s/1" % (Num, MR), tf.get_default_graph())
    writer_2 = tf.summary.FileWriter("logs/DPA/%s/c%s/2" % (Num, MR), tf.get_default_graph())

    sess.run(tf.global_variables_initializer())
    if tl.files.load_and_assign_npz(
            sess=sess, name=checkpoint_dir+'/g_{}_1.npz'.format(tl.global_flag['mode']), network=t1) is False:
...
###============================= TRAINING ===============================###
    ...
    ##step1###
    for epoch in range(0, n_epoch_dpa_1+1):
        ...
        for idx in range(0, int(len(train_hr_img_list)//batch_size)):
            ...
            errt1, errt1_1, errt1_2, __, summary = sess.run(
                [g_loss_t1, ade_loss_t1, tv_loss_t1, t1_optim, merged],
                {y1_image: y_fullimg, t_target_image: b_imgs})


            if n_iter_DPA % 500 == 0:
                print(
                    "Epoch1 [%2d/%2d] %4d time:%4.4fs,t1_loss: %.4f(t1_ade:%.4f, tv_loss:%.4f)" % (
            ...
            total_t1_loss += errt1
            total_mse_loss += errt1_1
            n_iter_DPA += 1
            sum1 += 1

        log = "[*] Epoch1: [%2d/%2d] time: %4.4fs, t1_loss: %.8f, ade_loss:%.8f" % (
        epoch, n_epoch_dpa_1, time.time() - epoch_time, total_t1_loss / n_iter_DPA, total_mse_loss / n_iter_DPA)
        print(log)
        ...
        if epoch % 1 == 0:
            out_T1 = sess.run([t1_.outputs], {y1_image: y_fullimg_sample})  # ; print('gen sub-image:', out.shape, out.min(), out.max())
            print("[*] save images")
            out_T1 = np.reshape(out_T1, [1, 128, 128, 1])
            out_T1_inv = tl.prepro.threading_data(out_T1, fn=inv_norm)
            tl.vis.save_images(out_T1_inv.astype(np.uint8), [1, 1], save_dir_DPA_1 + '/train_%d_T1.png' % epoch)
            psnr = compare_psnr(out_T1, sample_imgs_256.reshape([1, 128, 128, 1]))
            print("valid_psnr:%.4f" % psnr)


        ## save model
        if (epoch != 0) and (epoch % 1 == 0):
            tl.files.save_npz(t1.all_params, name=checkpoint_dir + '/g_{}_1.npz'.format(tl.global_flag['mode']), sess=sess)



    #step2###
    for epoch in range(0,n_epoch_dpa_2+1):
        ...
        for idx in range(0, int(len(train_hr_img_list)//batch_size)):
            ...
            errt2, errt2_1, errt2_2, errM, ___, summary = sess.run(
                [g_loss_t2, ade_loss_t2, meas_loss, mse_loss, t2_optim, merged],
                {y1_image: y_fullimg, t_target_image: b_imgs})

            if n_iter_DPA % 500 == 0:
                print(
                    "Epoch2 [%2d/%2d] %4d time:%4.4fs,t2_loss: %.4f(t2_ade: %.4f, meas: %.4f),mse_loss:%.4f" % (
                    epoch, n_epoch_dpa_2, n_iter_DPA, time.time() - step_time, errt2, errt2_1, errt2_2, errM))
